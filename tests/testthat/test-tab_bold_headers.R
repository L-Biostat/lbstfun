test_that("bold_headers applies style to column headers", {
  ...
  header_styles <- header_styles$styles[header_styles$locname == "columns_columns"]
...
test_that("tab_bold_headers applies bold style to column and spanning headers", {
  tbl <- gt::gt(head(mtcars)) %>%
  ...
  spanner_weights <- styles_df$styles[styles_df$locname == "columns_groups"] |>
    lapply(function(x) x$cell_text$weight) |>
    unlist()
